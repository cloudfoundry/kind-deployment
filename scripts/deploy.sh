...
helm upgrade --install k8s-rep oci://ghcr.io/cloudfoundry/k8s/helm/k8s-rep:$K8S_REP_VERSION --set-file ca_crt="$CERTS_DIR/ca.crt" --set "stacks.cflinuxfs4=ghcr.io/cloudfoundry/k8s/cflinuxfs4:1.300.0"
...
helm upgrade --install policy-agent oci://ghcr.io/cloudfoundry/k8s/helm/policy-agent:$POLICY_AGENT_VERSION
